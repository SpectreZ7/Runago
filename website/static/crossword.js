    ...
    toHtml: function (grid, show_answers) {
        ...
        for (var r = 0; r < grid.length; r++) {
            ...
            for (var c = 0; c < grid[r].length; c++) {
                var cell = grid[r][c];
                if (cell == null) {
                    html.push("<td class='no-border'>&nbsp;</td>");
                } else {
                    var css_class = "crossword-cell";
                    var is_start_of_word = (cell['across'] && cell['across']['is_start_of_word']) || (cell['down'] && cell['down']['is_start_of_word']);
                    var number = is_start_of_word ? label++ : "";

                    html.push("<td class='" + css_class + "' data-number='" + number + "'>");
                    var inputValue = show_answers ? " value='" + cell['char'] + "'" : "";
                    html.push("<input type='text' maxlength='1' class='crossword-input' data-correct='" + cell['char'] + "'" + inputValue + " />");
                    html.push("</td>");
                }
            }
            html.push("</tr>");
